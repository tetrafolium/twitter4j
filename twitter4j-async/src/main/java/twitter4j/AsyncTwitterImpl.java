...
class AsyncTwitterImpl extends TwitterBaseImpl implements AsyncTwitter {
    ...
    /*package*/
    AsyncTwitterImpl(final Configuration conf, final Authorization auth) {
        super(conf, auth);
        twitter = new TwitterFactory(conf).getInstance(auth);
    }

    @Override
    public void addListener(final TwitterListener listener) {
    ...
    public void getMentions() {
        getDispatcher().invokeLater(new AsyncTask(MENTIONS_TIMELINE, listeners) {
            @Override
            public void invoke(final List<TwitterListener> listeners) throws TwitterException {
    ...
    public void getUserTimeline(final String screenName, final Paging paging) {
        getDispatcher().invokeLater(new AsyncTask(USER_TIMELINE, listeners) {
            @Override
            public void invoke(final List<TwitterListener> listeners)
...
, final String streetAddress) {
    ...
    abstract class AsyncTask implements Runnable {
        ...
        AsyncTask(final TwitterMethod method, final List<TwitterListener> listeners) {
            this.method = method;
